	...
	if (Locations.Num() > 0)	// Locations.IsValidIndex(0)
	{
		if (MonsterTable)
		{
			...
			if (Manager)
			{
				...
				FStreamableDelegate Delegate = FStreamableDelegate::CreateUObject(this, &ARogueGameModeBase::OnMosterLoaded, SelectedRow->MonsterId, Locations[0]);
...
void ARogueGameModeBase::OnMosterLoaded(FPrimaryAssetId LoadedId, FVector LoadLocation)
{
	...
	if (Manager)
	{
		...
		if (MonsterData)
		{
			AActor* NewBot = GetWorld()->SpawnActor<AActor>(MonsterData->MonsterClass, LoadLocation, FRotator::ZeroRotator);
			if (NewBot)
			{
	...
	DrawDebugSphere(GetWorld(), LoadLocation, 50.0f, 20, FColor::Blue, false, 60.0f);
}
